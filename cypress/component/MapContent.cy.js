...
describe('MapContent', () => {
  ...
  it('Workflow testing', () => {
    ...
    cy.get('.mapcontent')
      .invoke('attr', 'style', 'height: 880px')
      .should('have.attr', 'style', 'height: 880px');
    ...
    cy.get('html').screenshot(
      'MapContent.cy.js/base/cypress/component/MapContent.cy.js/MapContent_1pane',
    );
    ...
    //Test searching with uberon id wich should display a pop up with anatomical name
    cy.get(
      '[style="height: 100%;"] > [style="height: 100%; width: 100%; position: relative;"] > .settings-group > :nth-child(1)',
    )
      .should('exist')
      .click();
    ...
    cy.get('.pane-1 .toolbar > .toolbar-flex-container > .el-select > .el-select__wrapper')
      .should('exist')
      .click();
    cy.get(
      '.pane-1 .toolbar > .toolbar-flex-container > .el-select .viewer_dropdown ul > li',
    ).should('have.length', 2);
    cy.get(
      '.pane-1 .toolbar > .toolbar-flex-container > .el-select .viewer_dropdown ul > :nth-child(1)',
      { timeout: 60000 },
    ).click();
    ...
    cy.get('.connectivity-card-container > .connectivity-card').should(
      'have.length.greaterThan',
      0,
    );
    cy.get('.connectivity-card-container > .connectivity-card').first().click();
    cy.get(':nth-child(1) > .connectivity-info', { timeout: 45000 }).should(
      'contain',
      'neuron-type-aacar',
    );
    ...
    //Dataset 76
    cy.get(
      '.sidebar-content-container > .el-card__header > .header > .search-input-container > .el-input > .el-input__wrapper',
    )
      .should('exist')
      .type('10.26275/llxf-edeh');

    //Search
    cy.get('.sidebar-content-container > .el-card__header > .header > .el-button--primary')
      .should('exist')
      .click();
    ...
    cy.get('@metadata').then((metadata) => {
      cy.intercept(
        '/sparc-api/s3-resource/999/1/files/derivative/sub-54-5/scaffold/54-5_metadata.json?s3BucketName=pennsieve-prod-discover-publish-use1',
    ...
    cy.get('@primitive').then((primitive) => {
      cy.intercept(
        '/sparc-api/s3-resource/999/1/files/derivative/sub-54-5/scaffold/cube_2.json?s3BucketName=pennsieve-prod-discover-publish-use1',
        { statusCode: 200, body: primitive },
      ).as('scaffoldResponse');
    });
    ...
    cy.get('@simulation_ui').then((simulation_ui) => {
      cy.intercept('/sparc-api//sim/dataset/999', { statusCode: 200, body: simulation_ui });
    });
    ...
    cy.get('.icon-group.el-row .el-popover:visible .el-row')
      .contains('Four panes')
      .should('exist')
      .click();
    ...
    cy.get('.viewing-mode-selector > .toolbar-dropdown')
      .as('changeViewingMode')
      .trigger('mouseenter');
    cy.get('.toolbar-dropdown-popper .el-dropdown-menu__item > h5')
      .as('viewingModes')
      .contains('Annotation')
      .click();
    ...
    // UI
    cy.get('html').screenshot(
      'MapContent.cy.js/base/cypress/component/MapContent.cy.js/MapContent_4panes',
    );
    Object.entries(settings).forEach(([setting, index]) => {
