    ...
    for (let key in list) {
      if (condition.taxo === list[key].taxo) {
        if (condition.biologicalSex && list[key].biologicalSex) {
          if (condition.biologicalSex === list[key].biologicalSex) return key;
        } else {
          return key;
        }
      }
...
const getBodyScaffoldInfo = async (sparcApi, species) => {
  ...
  if (response.ok) {
    ...
    if (species === 'rat') {
      ...
      url =
        'https://mapcore-bucket1.s3.us-west-2.amazonaws.com/WholeBody/27-4-23-human/human_body_metadata.json';
    }
  }
...
const intersectArrays = (arr1, arr2) => {
  ...
  return arr1.filter((x) =>
    typeof x === 'string' ? lowerArr2.includes(x.toLowerCase()) : lowerArr2.includes(x),
  );
};
