  ...
  methods: {
    ...
    getContextFileFromScicrunch: async function (datasetId, scaffoldPath) {
      ...
      let results = await fetch(
        `${this.settingsStore.sparcApi}/dataset_info/using_multiple_discoverIds/?discoverIds=${datasetId}`,
      )
        .then((response) => response.json())
        .then((data) => {
          // get the context info from the response
          if (data.numberOfHits === 1) {
            // check if there is only one hit (We don't want to use the data if there are multiple hits)
            ...
            // If there are multiple context files, find the one that matches the scaffold path
              ...
              let search = this.findContextInforForFilePath(
                data.results[0]['abi-context-file'],
                scaffoldPath,
              );
              if (search) {
                return {
